...
class GetStimLockedBins:
    ...
    def get_start_stop_cols(self):
        possible_pairs =[['constrained_start', 'constrained_stop'], ['start_fix', 'stop_fix'], ['start_roi', 'stop_roi']]
    ...
    def format_durations(self, start_dur, stop_dur):
        merged = pd.concat([start_dur, stop_dur]).set_index('bin', append=True)
        group_by = merged.index.names.difference(['fix_id'])
        summed= merged.groupby(level=group_by).sum()
        return summed
    ...
    def fill_durations(column, start_dur, stop_dur):
        ...
        if not matching_start_dur.empty:
            column.loc[matching_start_dur.index] = column.loc[matching_start_dur.index].fillna(matching_start_dur['duration'])
        if not matching_stop_dur.empty:
            column.loc[matching_stop_dur.index] = column.loc[matching_stop_dur.index].fillna(matching_stop_dur['duration'])
        return column
