...
class CalculateEntropy:
    ...
    def __init__(self
        ...
        self._transition_counts, self._metadata = CreateTransitionMatrices(fixation_roi, target_roi,
    ...
    def format_entropies(self):
        ...
        df = pd.DataFrame(self.normalized_entropies, columns=['transition_entropy'],
                          index=pd.MultiIndex.from_tuples(self._metadata))
    ...
    def calc_entropy(self):
        ...
        for prob_matrix, distributions in zip(self.transition_probabilities, self.stationary_distributions):
            ...
            if len(distributions) > 0:
                # Fill zero probabilities with one to avoid log2(0) warnings. log2(1) = 0
                ...
                # Total entropy is the sum of each transition entropies
                total_entropy = np.sum(entropy_matrix)
                raw_entropies.append(total_entropy)
            else:
                # Append error flag if stationary distributions couldn't be calculated
    ...
    def calculate_stationary_entropy(self):
        ...
        for distributions in self.stationary_distributions:
            if len(distributions) > 0:
                ...
                stationary_entropy = sum([x * np.log2(1 / x) for x in distributions if x > 0])
